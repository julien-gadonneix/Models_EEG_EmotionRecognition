...
class PrimaryCap(nn.Module):
    def __init__(self, inputs, dim_capsule, n_channels, kernel_size, strides, padding, model_version, dev):
        ...
        if model_version == 'v2':
            self.caps2 = nn.Conv2d((dim_capsule*n_channels)+inputs, 256, kernel_size=1, stride=1, padding='valid', device=dev)
...
class CapsEEGNet(nn.Module):
    def __init__(self, nb_classes, Chans):
        ...
        self.primaryCaps = PrimaryCap(8*2, 8, 32, (1, 6), 1, 'same', 'v2', None) # ReLU activation after the convolutional layer not present but residual present
...
class TCNet(nn.Module):
    def __init__(self, nb_classes, Chans, dev0=None, dev1=None):
        ...
        self.primaryCaps = PrimaryCap(d, 8, 48, 6, 1, 'same', 'v0', dev1)
